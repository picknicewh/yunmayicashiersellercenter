...
public class TagAliasOperatorHelper {
                ...
                case DELAY_SEND_ACTION:
                    if(msg.obj !=null && msg.obj instanceof  TagAliasBean){
                        ...
                        if(context!=null) {
                            ...
                            G.log("#unexcepted - context was null");
                        }
                    }else{
                        G.log("#unexcepted - msg obj was incorrect");
                    }
                    break;
            }
        }
        ...
        if(tagAliasBean.isAliasAction){
            switch (tagAliasBean.action){
                ...
                case ACTION_SET:
                    ...
                    break;
                default:
                    G.log("unsupport alias action type");
                    return;
            }
        }else {
            switch (tagAliasBean.action) {
                ...
                case ACTION_CLEAN:
                    ...
                    break;
                default:
                    G.log("unsupport tag action type");
                    return;
            }
        }
    }
    ...
    public void onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        G.log("action - onTagOperatorResult, sequence:"+sequence+",tags:"+jPushMessage.getTags());
        G.log("tags size:"+jPushMessage.getTags().size());
        init(context);
        ...
        if(jPushMessage.getErrorCode() == 0){
            G.log("action - modify tag Success,sequence:"+sequence);
    ...
    public void onCheckTagOperatorResult(Context context, JPushMessage jPushMessage){
        int sequence = jPushMessage.getSequence();
        G.log("action - onCheckTagOperatorResult, sequence:"+sequence+",checktag:"+jPushMessage.getCheckTag());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = tagAliasActionCache.get(sequence);
        if(tagAliasBean == null){
    ...
    public void onAliasOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        G.log("action - onAliasOperatorResult, sequence:"+sequence+",alias:"+jPushMessage.getAlias());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = tagAliasActionCache.get(sequence);
        if(tagAliasBean == null){
        ...
        if(jPushMessage.getErrorCode() == 0){
            G.log("action - modify alias Success,sequence:"+sequence);
            tagAliasActionCache.remove(sequence);
