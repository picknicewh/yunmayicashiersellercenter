...
public class GoodsListAdapter extends CommonRecyclerAdapter<GoodsDetailInfo>{
    public GoodsListAdapter(Context context, List<GoodsDetailInfo> goodsInfo, int layoutId) {
        super(context, goodsInfo, layoutId);
    }

    @Override
    public void convert(CommonRecyclerHolder holder, GoodsDetailInfo goodsInfo) {
        ...
        tv_price.setText("￥"+ PriceTransfer.chageMoneyToString(goodsInfo.getPrice()));
        tv_name.setText(goodsInfo.getTitle());
        GlideUtils.loadImageView(mContext, goodsInfo.getImage_url(),iv_image);
        iv_off_shelf.setVisibility(goodsInfo.getItem_status()==1? View.GONE:View.VISIBLE);
